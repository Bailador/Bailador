...
is-deeply get-psgi-response($p6w-app, 'GET',  '/foo'),  [200, ["Content-Type" => "text/html;charset=UTF-8"], 'foo text'],       'route GET /foo returns content';
is-deeply get-psgi-response($p6w-app, 'POST', '/bar'),  [200, ["Content-Type" => "text/html;charset=UTF-8"], 'peti bar'],       'route POST /bar returns content';
...
is-deeply get-psgi-response($p6w-app, 'GET',  '/params/bar'),   [200, ["Content-Type" => "text/html;charset=UTF-8"], 'a happy bar'],       'route GET /params/bar returns content';
is-deeply get-psgi-response($p6w-app, 'GET',  '/regexes/bar'),  [200, ["Content-Type" => "text/html;charset=UTF-8"], 'a happy bar'],       'route GET /regexes/bar returns content';
...
is-deeply get-psgi-response($p6w-app, 'GET', '/header1'), [ 200, ["X-Test" => "header1", "Content-Type" => "text/html;charset=UTF-8" ], "added header X-Test" ], 'ROUTE GET /header1 sends an extra header';

is-deeply get-psgi-response($p6w-app, 'GET',  '/header2'),  [ 200, ["X-Again" => "header2", "Content-Type" => "text/html;charset=UTF-8"], 'added header X-Again' ], 'ROUTE GET /header2 sends an extra and does not include headers from previous requests';
